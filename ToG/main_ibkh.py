...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str,
                        default="pubmed1000", help="choose the dataset.")
    ...
    args.opeani_api_keys = api_key
    datas, question_string = prepare_dataset(args.dataset)
    te_string = "topic_entity" if args.dataset != "pubmed1000" else "qid_topic_entity_embeddings"
    print("Start Running ToG on %s dataset." % args.dataset)
    for i, data in tqdm(enumerate(datas), desc="processing questions"):
        if i>=1:
            break
        question = data[question_string]
        topic_entity = data[te_string]
        ...
        for depth in range(1, args.depth+1):
            ...
            for entity in topic_entity:
                if entity!="[FINISH_ID]":
                    ...
                    try:
                        if isinstance(topic_entity[entity], dict):
                            retrieve_relations_with_scores = relation_search_prune(entity, topic_entity[entity]['qid_label'], pre_relations, pre_heads[i], question, args)  # best entity triplet, entitiy_id
                        else:
                        ...
                        print(f"compromised question: {question}")
                        break
            ...
            pre_relations = [labels2rels[r] if r in labels2rels else r for r in pre_relations]
            if flag:
